...
const Attendance = () => {
      ...
      <div className="flex justify-between items-end mb-4">
        ...
        <div className="relative">
           <form className="form relative" onSubmit={(e) => e.preventDefault()}>
                <button className="absolute left-2 -translate-y-1/2 top-1/2 p-1" type="button" title="Search">
                <svg
                    width="17"
                    height="16"
                    fill="none"
  ...
  <tr>
    <th colSpan={6} className="text-center py-2 font-bold">
      Math 101 | A Section | CSE
    </th>
    <th className="text-right py-2 px-4 flex items-end justify-end">
      <button
        onClick={() => setShowAbsentees((prev) => !prev)}
        className="flex items-end justify-center gap-2 text-black font-bold py-2 px-4 rounded-md border border-gray-300 hover:bg-gray-200 duration-300 fixed-width-button"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="h-5 w-5"
  ...
  <tr>
    ...
    <th className="py-2 px-4">Status</th>
      ...
      <div className="mt-6 flex justify-center">
        <button className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-6 rounded-lg">Submit</button>
      </div>
    </div>
    </div>
  );
